...
object EmergencyCallHelper {
    ...
    /**
     * Automatically opens WhatsApp chat with the emergency contact with
     * the emergency distress message and live Google Maps tracking link pre-filled.
     * Supports regular WhatsApp (com.whatsapp), WhatsApp Business (com.whatsapp.w4b),
     * and web browser fallback.
     */
    fun sendWhatsAppEmergencyAlert(context: Context, rawPhone: String, message: String) {
        ...
        try {
            ...
            Log.i(TAG, "Dispatched WhatsApp emergency message to $cleanPhone")
            return
        } catch (e: Exception) {
        ...
        try {
            ...
            Log.i(TAG, "Dispatched WhatsApp Business emergency message to $cleanPhone")
            return
        } catch (e: Exception) {
            Log.w(TAG, "WhatsApp Business not available: ${e.message}")
        }

        // Final fallback: standard browser / system handler
        try {
